...
Learning mode depends downward only: on [knowledge/](../knowledge/INDEX.md), on [templates/](../os/WHICH-DOCUMENT.md), on the loop files in [os/](../os/OPERATING-LOOP.md), and on the Conductor's question banks, all read-only. Nothing outside `learn/` knows this layer exists. Delete the folder and the OS loses nothing but the curriculum.
