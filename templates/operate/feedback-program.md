...
**What checks this.** On the default path, where you fill this copy in an editor and a named human signs the gate form, Gate 6 is signed by the product owner and the sponsor, per [os/STAGE-GATES.md](../../os/STAGE-GATES.md), and editing the document afterwards does not stale that signature: no hash binds it to the bytes. If you are running the optional `pmos` runtime, `python3 tools/init_product.py <product> --add` has already stamped this copy's artifact block from the frontmatter above, and approving Gate 6 with `pmos gate --bank-id operate` records the copy's revision, so a later edit does stale that approval until the gate is proved again. Either way, no tool checks whether what you write is right; the people who sign the gate do. This paragraph is generated by `python3 tools/what_checks.py`.
