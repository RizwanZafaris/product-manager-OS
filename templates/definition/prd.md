...
Filled example: [Expense Copilot](../../examples/expense-copilot-prd.md), a complete fill: every section of the spine, 0 to 13, plus sign-off and the exit gate, with the companions this fictional build does not have named as gaps rather than left blank
...
| # | Objective | Evidence | Metric | Metric ID | Baseline | Target | Metric owner | Measured where |
|---|---|---|---|---|---|---|---|---|
| O1 | | E[n] in [../discovery/evidence-note.md](../discovery/evidence-note.md) | | M-[n] in [../operate/metrics-dictionary.md](../operate/metrics-dictionary.md) | | | | |
| O2 | | | | | | | | |
...
Two columns carry the lineage in, so a reviewer never has to guess where an
objective came from or which number it means. Evidence is the note ID in
../discovery/evidence-note.md, or the synthesis row, that says this problem is
real; an objective with no evidence ID is a preference. Metric ID is the row ID
in ../operate/metrics-dictionary.md that fixes the formula, grain and filters;
without it, two people read one target as two different numbers. Cite the IDs
rather than restating them: markdown cannot deep-link a table row.
...
| # | Story | Persona | Priority (must / should / later) | Acceptance criteria ID | Verified by (test ID, filled at BUILD) |
|---|---|---|---|---|---|
| US1 | As a [persona], I want [action], so that [outcome]. | | must | AC-[n] in [acceptance-criteria.md](acceptance-criteria.md) | [test ID when it exists] |
| US2 | | | | | |
...
The last column stays empty until BUILD and then carries the test ID that ran
that criterion. It is the step that lets a reader get from an objective to a
result without opening three other documents, and it is a summary of the
traceability matrix in [frd.md](frd.md), which stays the place the mapping is
maintained.
...
| # | Capability | What it does, in one sentence | Objective it serves | Story it serves | Release slice | Detail |
|---|---|---|---|---|---|---|
| F1 | | | O[n] | US[n] | S[n] | FR-[n] in [frd.md](frd.md) |
...
The objective column closes the loop the objectives table opens: a capability
that serves no objective is scope nobody agreed to fund, and an objective with
no capability is a target nothing ships against. The release slice column names
which increment carries the row, and the slice table below says what that slice
waits on. Both are summaries; the roadmap and the dependency register stay the
source of truth.
...
| Slice | What it delivers | Capabilities | Depends on | Dependency register ID | Planned release |
|---|---|---|---|---|---|
| S1 | | F[n] | [external deliverable, or "none"] | [row # in the dependency register, or "n/a"] | [roadmap row, or the target period] |

<details open>
<summary>Guidance</summary>

One row per increment that can be released on its own. This is the smallest
thing that answers "what has to be true before this capability can ship", and
it is the reason a launch criterion in section 8 can name what it covers.

Write "none" in the depends-on column rather than leaving it empty. An empty
cell reads as unexamined; "none" reads as answered, and a reviewer can tell the
difference at a glance. Anything not "none" carries its row number in
[../execution/dependency-register.md](../execution/dependency-register.md),
because the commitment, the contact and the date are maintained there and
copying them here guarantees they drift. The planned-release cell names the row
in [../planning/roadmap.md](../planning/roadmap.md) that carries this slice.

If this product ships in one go, say so in one row. A slice table is not a
reason to invent phases.
...
| # | Criterion | Covers (slice or objective) | Verified by (artifact or test ID) | Owner |
|---|---|---|---|---|
| L1 | All "must" stories pass their acceptance criteria | S[n] | [acceptance-criteria.md](acceptance-criteria.md) | |
| L2 | Non-functional targets met or waived by their owner | S[n] | [nfr.md](nfr.md) | |
...
The covers column is what turns this table into the end of a trace rather than a
separate checklist. A reader who started at an objective arrives here knowing
which slice this row releases, and the verified-by cell names the test ID or the
artifact that settles it. A criterion covering nothing is either dead or the
slice table is missing a row.
...
system reaches this document. Read the triggers and answer every row: Yes or No,
with the reason in the last column. Open the templates the Yes rows name.

Answer every row, including the ones that do not apply. A blank cell is
indistinguishable from a row nobody read, and "No, no personal data leaves the
tenant" is a claim someone can challenge at the gate while a blank is not. A No
with a reason is the record of what you considered and did not need; that is the
point of keeping the untriggered rows.
...
| Trigger, if this is true of your product | Open this | Stage | Applies (Yes / No) | Why |
|---|---|---|---|---|
...
**Baseline:** [the version in the header that Gate 2 approved, for example v1.0]
**Changes since baseline:** [the CR numbers from the change history below, comma separated, or "none"] · Register: [change-request.md](../execution/change-request.md)

| Role | Name | Date | Revision signed | What they are signing |
|---|---|---|---|---|
...
| Version | Date | What changed (rows by ID) | CR | Approvals re-taken | Approvals unaffected |
|---|---|---|---|---|---|
| [n] | [YYYY-MM-DD] | [the row IDs that moved: O2, AC-7, S1] | CR-[n] | [roles that signed again] | [roles whose signature still stands, and why] |
...
The revision column is what stops a signature outliving the document it signed.
A name and a date say somebody agreed; only the revision says what they agreed
to. On the runtime path `pmos gate --bank-id define` records the copy's revision
and a later edit stales the approval, as the paragraph at the top of this file
explains; on the default path this column is the only record there is, so it is
not optional there.

The change history is the other half. A change that does not say which approvals
it re-took leaves every reader to assume the whole sign-off block still holds,
which is how a signed PRD quietly becomes a signed-in-2024 PRD. Name the rows
that moved by ID, the CR that carried them, who signed again, and who did not
have to and why. "Design lead unaffected: no user-facing surface changed" is a
sentence somebody can disagree with; silence is not.

The CR itself, its impact analysis and its own approval table live in
../execution/change-request.md. This table is the index into it.
...
- [ ] Every objective names an evidence ID and a metric-dictionary ID
- [ ] Every must story carries an acceptance criteria ID
- [ ] Functional scope rows each trace to a story, an objective, and a release slice
- [ ] Every release slice names its dependencies or says "none"
...
- [ ] Launch criteria are all checkable, each with an owner, and each names what it covers and what verifies it
...
- [ ] Every companion row is answered Yes or No with a reason, and the Yes rows are open
...
- [ ] The sign-off block names real people and the revision each signed, and the Gate 2 approver did not write this document
- [ ] The baseline is named, and every change since it names its CR and the approvals it re-took
