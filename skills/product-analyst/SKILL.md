...
For each source consulted, fill one [evidence note](../../templates/discovery/evidence-note.md). The load-bearing rule: every note carries the source's evidence in the form the source has it. Where the source is words, that is its key sentence verbatim, in quotation marks, because paraphrase drifts and a quote is checkable later; if no single sentence in the source carries the claim, the claim is not in that source, and the note says what the source actually supports. Where the source is a metric export, a dataset or a query result, it is the query, the filters, the denominator and the period, because a count nobody can reproduce is a rumour with a number in it. Where it is a watched session, it is the session and timecode and what was observed, in plain words. Quotation marks are never used for a sentence nobody said. Sources read and found irrelevant get a one-line note too: what was checked and why it did not apply. Silence about a dead end is how the next reader repeats it.
