...
| M-003, share of eligible reports drafted, pooled across active add-on accounts | Input | 47%, 555 of 1,180 (N65) | Target 50% (N72); held near Phase 1's 54% as the account base widened past the six design partners (metrics review section 2) | Product analytics |
...
| M-011, add-on MRR | Diagnostic; KR4 | $15,900 (N68) | Target $30,000; actual $15,900; KR4 scored 0.53 (N73) | Billing |
...
- **Why this one:** M-006 sat at 76% against a 78% target, so the product itself is not the ceiling on M-002; the metrics review located the gap at the price step of the funnel, where M-007 pooled at 2.7% against a 6.0% ship bar (N57, N61). That is the input with the most headroom relative to what the product already delivers, and it is the only input the pivot decision (D6) reopened. The one measured reference point for an add-on's conversion at Ledgerline is the prior Bills Automation add-on, 4.1% of exposed accounts paid within 14 days (N39); EXP-1's 2.7% sat below it, and EXP-2's 6.0% bar is a decision threshold set by the pre-declared ship rule (N57), not a second observed rate.
...
- **Worth, before cost:** at the 6.0% success threshold, about 170 of the 2,827 exposed accounts (2,827 x 6.0%) would convert. At the median S-1 account's 7 reports filed a month (N33) and $2.40 per drafted report, the monthly bill per converted account ranges from about $7.90 at the observed 47% drafted share (N65, M-003) to about $16.80 if every filed report were drafted; across 170 converted accounts that is about $1,340 to $2,856 a month, roughly $16,000 to $34,000 a year. N90's $26-a-month figure uses the mean of 11 reports a month, not the median, and overstates the typical account's bill even at a 100% drafted share.
...
- **Buyer-facing cost:** the median account's usage price is about $7.90 to $16.80 a month (see Worth, above), the range set by the 47% observed drafted share (N65, M-003) up to every filed report being drafted, against a $149-a-month plan, about 5% to 11% of it (N27), and against Cinderwick's $8-per-seat benchmark. The data sheet's N90 row still derives its $26 figure from the mean of 11 reports a month rather than the median of 7 (N33); Open: N90 needs relabelling as the mean-account figure, or recomputing at the median, before it is cited as a buyer-facing number again. No buyer has been asked this specific number: 5 of 8 advisory board members asked for a per-report price in the abstract (N78), and 0 of 6 design-partner interviews asked any price (N40); willingness to pay at $2.40 is unvalidated.
...
| Targets with no baseline | A committed lift with no documented starting number | This plan sets no next-cycle M-002 or MRR target itself, and section 2 records that as an open item rather than leaving it implied by the OKR sheet; when set, both will be deltas from the dated baselines already on this page, 79 accounts and $15,900 at 2026-12-18 (N64, N68) |
