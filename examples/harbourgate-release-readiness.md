...
| 3 | 2026-07-29 | Later attempt, recorded 2026-08-26: GO WITH CONDITIONS for the Tidewater store cohorts: DEP-5 delivered and G1 closed before cohort 1 (met 2026-07-30 and 2026-07-31) (separate document) |
| 4 | 2026-08-26 | Later attempt, recorded 2026-08-26: GO for the Kestrel legacy connector cohorts (separate document) |

Addendum, 2026-08-26: rows 3 and 4 were both added to this table on this date, after the fact, when attempt 4's document was filed; row 3's own attempt (2026-07-29) was not logged here contemporaneously, which is why its "recorded" date matches row 4's rather than its own attempt date.

## 1. Features

- [ ] Everything in the PRD's launch scope for cohort 1 is built, and nothing extra shipped unreviewed: not literally true, exception carried to condition C1. HARBOURGATE-S1 (pay by card, web and app), S3 (decline with reason class and one retry), S4 (per-provider routing flag with two-person change), S5 (refund to the originating provider), S6 (settlement ingestion and reconciliation), S7 (legacy table shape written unchanged), S8 (decline event stream), S10 (step-up authentication), S11 (capture on dispatch, void on fulfilment failure) are built. S2 and S9 (kiosk payment and kiosk fallback) are Tidewater's stories and are not in this cohort's scope; S12 (settlement-file-absent alert) is condition C1 below, not yet shipped at sign-off.
...
- [x] Every checklist box above is checked, or its exception sits in the known-issues table or a named condition with an owner and a date: sections 4, 5 and 7 checked; in section 1 the scope and acceptance-criteria boxes are unticked with their exception carried to condition C1, and in section 2 the edge-case box is unticked with its exceptions in section 3, and the AI box is N/A; section 3 holds the three open exceptions
...
- [x] Every Severity cell is a canonical id from the testing strategy ladder, or `not a defect:` followed by one of the four gap classes named in section 3: row 1 is S3; row 2 is an untested path and row 3 is unstaffed support
- [x] No known-issues row is S1, and every S2 row names the approver who accepted it and why, or it has moved to a condition: no S1 and no S2 rows in section 3
