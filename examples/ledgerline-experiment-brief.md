...
**Proposed for the next two-arm experiment, 2026-12-22 (EXP-2 is single-arm, N91, so this refinement is deferred until a controlled test exists):** tightening the decision rule for a future two-arm experiment by requiring the anchored arm to clear control by the full 2.5-point minimum detectable effect (N55) on Ship, and narrowing "either arm" to the arm under test on Iterate and Kill. EXP-2 itself carries only the one carry-forward that does apply to a single-arm design, M-010 as a Ship-condition guardrail, already written into EXP-2's own counter-metrics (N92). The 2,080-per-arm power observation below is the reason the two-arm refinement is proposed; it is a limitation of this experiment's sizing, not a rule this experiment was run against. None of this refinement changes EXP-1's own decided outcome: both arms fell under 4.0% and every guardrail held, so the result is KILL under N57's wording as pre-declared.
