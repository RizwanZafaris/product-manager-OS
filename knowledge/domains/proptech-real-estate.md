...
This domain runs two loops that rarely share a codebase and never share a regulator: the discovery loop of search, leads, and showings, and the transaction loop of financing and conveyancing that moves title and money. A feature that speeds up search can slow the transaction if it produces leads that title and lending cannot process at the same rate. The second distinctive fact: "neutral" fields rarely are. Address, zip code, name, and photo correlate tightly with race, national origin, and familial status, and fair-housing law does not require intent: disparate-impact liability exists, though Inclusive Communities (2015) demands a robust causal link between the challenged policy and the disparity, so a bare statistical gap is not enough and a feature is not safe just because nobody meant harm. Outside the US MLS and title-insurance model, the transaction loop looks different again: across most of continental Europe a civil-law notary, not an escrow company, is the instrument that clears title, and software built around the American closing process breaks quietly the first time it meets one.

**Adjacent industries:** software for designing, estimating, building or paying for a building before it is finished reads [Construction and AEC](construction-aec.md). There the project, not the property, is the customer, pay applications and lien waivers make the payment flow a legal instrument, and the project record becomes evidence in claims. This card takes over once the building is listed, sold, leased, managed or financed.
...
**Filled in this repo:** [domain-proptech-real-estate-data-model.md](../../examples/domain-proptech-real-estate-data-model.md) fills the [data-model](../../templates/architecture/data-model.md) template directly for this domain, for Cobblestone Listings' home-search ranking feature: fields flagged for fair-housing proxy risk separately from PII sensitivity (postcode or ZIP, language preference, surname-derived signals, school-rating joins), a rule excluding proxy-flagged fields from ranking and ad targeting citing the 2022 US DOJ settlement with Meta over housing ad delivery, plus disclosure fields and MLS data-licence restrictions. For the other four bent templates, [sahulat-personas.md](../../examples/sahulat-personas.md), [harbourgate-integrations.md](../../examples/harbourgate-integrations.md), [ledgerline-gtm-plan.md](../../examples/ledgerline-gtm-plan.md) and [harbourgate-compliance-impact-assessment.md](../../examples/harbourgate-compliance-impact-assessment.md) remain the nearest reading for persona, boundary-register, cohort-evidence, and automated-decision shapes, though none is a four-person proptech loop.

<!-- generated: example-availability -->
**Example availability (generated).** Of the 5 templates this card bends, 5 have a filled example somewhere in this repository: [personas](../../templates/discovery/personas.md) (1), [data-model](../../templates/architecture/data-model.md) (3), [integrations](../../templates/architecture/integrations.md) (2), [gtm-plan](../../templates/planning/gtm-plan.md) (1), [compliance-impact-assessment](../../templates/operate/compliance-impact-assessment.md) (3). Written for this domain: [domain-proptech-real-estate-data-model.md](../../examples/domain-proptech-real-estate-data-model.md). Filled examples of [personas](../../templates/discovery/personas.md), [integrations](../../templates/architecture/integrations.md), [gtm-plan](../../templates/planning/gtm-plan.md), [compliance-impact-assessment](../../templates/operate/compliance-impact-assessment.md) exist, but none was written for this domain. This paragraph is generated by `python3 tools/example_availability.py`.
<!-- end generated: example-availability -->

**Worked example (ILLUSTRATIVE):** a slice of [data-model](../../templates/architecture/data-model.md), the template this domain bends hardest, for a fictional home-search ranking feature at a fictional brokerage-tech company, Cobblestone Listings. Every name and figure is invented.

| Field | Table | Used by | Proxy risk | Mitigation | Owner |
|---|---|---|---|---|---|
| applicant.last_name | tenant_screening | Screening-rank model v1 | Correlates with national origin; flagged for disparate-impact review under Inclusive Communities (2015) | Excluded from the ranking feature set; retained only for identity verification, never passed to the scoring function | [OPEN: name], Data Protection lead |
| listing.zip_code | search_ranking | Home-search relevance model | Correlates tightly with race and national origin in most US metros; using it directly for relevance risks disparate-impact exposure even with no intent | Replaced in the ranking feature set with commute-time and price-band features; zip retained for logistics only | [OPEN: name], Fair Housing compliance owner |
| applicant.photo | listing_lead | None (display only) | High risk if ever fed to a screening or scoring model; currently display-only | Access-controlled to prevent silent reuse in a future scoring feature; any proposal to score against it requires legal sign-off before Gate 2 | [OPEN: name] |
