...
**Conductor overlay:** this domain sharpens DISCOVER-1 (name the person, since the receiver, not the paying sender, usually experiences the failure), DESIGN-2 (integrations, because every correspondent bank and payout partner needs an owner and a failure behavior, de-risking included), DEFINE-8 (overlays, since money moving across borders is exactly the case this question exists to catch), and OPERATE-4 (cost to run, where held-transfer support volume is the operational load that matters).
