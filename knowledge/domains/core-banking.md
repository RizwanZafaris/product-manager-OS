...
**Filled in this repo:** [domain-core-banking-sla-slo-definition.md](../../examples/domain-core-banking-sla-slo-definition.md) fills the [sla-slo-definition](../../templates/delivery/sla-slo-definition.md) template directly for this domain, for Aldergate Core: a batch-completion SLI against a named cut-off before branch and app opening, held distinct from real-time posting availability and latency, with an error-budget policy that spends budget on a late batch even at 100% uptime and a monthly review with the bank's operational-resilience lead. For the other three bent templates, [harbourgate-migration-cutover-plan.md](../../examples/harbourgate-migration-cutover-plan.md) and [harbourgate-release-readiness.md](../../examples/harbourgate-release-readiness.md) remain the nearest reading, for the phased-cohort-with-drain shape and the GO WITH CONDITIONS decision, though both migrate a payment service, not a bank's own core, and carry no interest-accrual or standing-instruction lag. No filled data-model example in this repository models a bank's own core; [expense-copilot-data-model.md](../../examples/expense-copilot-data-model.md) is the nearest filled one for entity, key and retention discipline, and [harbourgate-system-design.md](../../examples/harbourgate-system-design.md) is near only for naming an owner per component.

<!-- generated: example-availability -->
**Example availability (generated).** Of the 4 templates this card bends, 4 have a filled example somewhere in this repository: [migration-cutover-plan](../../templates/delivery/migration-cutover-plan.md) (1), [data-model](../../templates/architecture/data-model.md) (3), [release-readiness](../../templates/delivery/release-readiness.md) (3), [sla-slo-definition](../../templates/delivery/sla-slo-definition.md) (3). Written for this domain: [domain-core-banking-sla-slo-definition.md](../../examples/domain-core-banking-sla-slo-definition.md). Filled examples of [migration-cutover-plan](../../templates/delivery/migration-cutover-plan.md), [data-model](../../templates/architecture/data-model.md), [release-readiness](../../templates/delivery/release-readiness.md) exist, but none was written for this domain. This paragraph is generated by `python3 tools/example_availability.py`.
<!-- end generated: example-availability -->

**Worked example (ILLUSTRATIVE):** a filled SLA/SLO pair for a fictional core, Aldergate Core, showing why uptime has to be defined separately for real-time posting and the overnight batch, the exact lie this card's own metrics table names against a single blended availability number.

| Service | SLO | Measurement window | Excludes | Owner |
|---|---|---|---|---|
| Real-time balance and posting API | 99.9% of minutes (target, ILLUSTRATIVE) | Rolling 30 days, continuous | Nothing; the definition deliberately does not exclude the nightly batch window, because a customer trying to pay rent at 2 a.m. does not care that batch is running | Head of Core Platform |
| End-of-day batch completion | Complete by 05:00 local, ahead of branch open | Daily | n/a | Batch Operations Lead |

A generic SLA template would collapse these into one "uptime" number. The sector fact that forces two rows is that a batch window finishing late but before branch open reports as healthy on a blended metric while it silently compresses every downstream reconciliation window that follows it.
