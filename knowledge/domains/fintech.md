...
**Filled in this repo:** the regulated module carries its own byte-exact worked example of the template inside it, [modules/regulated/examples/dispute-summary/PRD.md](../../modules/regulated/examples/dispute-summary/PRD.md), and this card deliberately does not duplicate it; no PRD example under `examples/` carries a run reg-gap-check output. The nearest filled artifact is [harbourgate-compliance-impact-assessment.md](../../examples/harbourgate-compliance-impact-assessment.md), which fills the one template this card names outside the module, [templates/operate/compliance-impact-assessment.md](../../templates/operate/compliance-impact-assessment.md); read its section 2 (applicable regulations, obligations triggered, evidence holder, owner) for table shape, since its actual regime is PCI DSS and UK GDPR on a payments checkout, not a financial-services licence or either instrument the regulated module currently cites. The financial-services sub-sector cards this pointer routes to carry their own filled examples where one has been written; each card's generated availability paragraph says which, so read that rather than trusting a count written here.

<!-- generated: example-availability -->
**Example availability (generated).** This card declares no templates that it bends, so there is nothing to report here. This paragraph is generated by `python3 tools/example_availability.py`.
