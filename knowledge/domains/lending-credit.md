...
**Filled in this repo:** [domain-lending-credit-risk-register.md](../../examples/domain-lending-credit-risk-register.md) fills the [risk-register](../../templates/execution/risk-register.md) template directly for this domain, for Harrow Credit: the R9 row on adverse-action reason codes that do not reflect what the model weighted, mitigated by a monthly 50-decline sample audit against Regulation B's specificity standard, plus disparate-impact monitoring under ECOA, collections conduct limits under the FDCPA and Regulation F, FCRA data-furnishing accuracy, model drift, and the UK FCA's BNPL regulation, with accepted risks signed by the CRO. For the other three bent templates, [harbourgate-business-rules.md](../../examples/harbourgate-business-rules.md) remains the nearest reading, for testable, numbered rule logic, though its rules are payment-decline logic, not adverse-action logic. No filled or planned example in this repo exercises eval-spec on a decisioning model; the nearest starting point is the unfilled template itself at [templates/ai/eval-spec.md](../../templates/ai/eval-spec.md).

<!-- generated: example-availability -->
**Example availability (generated).** Of the 4 templates this card bends, 4 have a filled example somewhere in this repository: [eval-spec](../../templates/ai/eval-spec.md) (2), [privacy-impact-assessment](../../templates/architecture/privacy-impact-assessment.md) (1), [risk-register](../../templates/execution/risk-register.md) (6), [business-rules](../../templates/definition/business-rules.md) (6). Written for this domain: [domain-lending-credit-risk-register.md](../../examples/domain-lending-credit-risk-register.md). Filled examples of [eval-spec](../../templates/ai/eval-spec.md), [privacy-impact-assessment](../../templates/architecture/privacy-impact-assessment.md), [business-rules](../../templates/definition/business-rules.md) exist, but none was written for this domain. This paragraph is generated by `python3 tools/example_availability.py`.
<!-- end generated: example-availability -->

**Worked example (ILLUSTRATIVE):** a risk-register row (fills [risk-register](../../templates/execution/risk-register.md) section 2) for a BNPL underwriting model:

| # | Risk | Category | L | I | Score | Response | Mitigation and trigger | Owner | Review date |
|---|---|---|---|---|---|---|---|---|---|
...
At score 6 the register's own threshold requires an active mitigation with a date, not a watching brief. This is the row a lending product adds on top of the register's generic delivery and security rows, because the underlying event is a compliance failure, not an outage, and its score comes from the probability of an unreconstructed reason surfacing in a regulator's sample.
