...
**Conductor overlay:** this domain sharpens DEFINE-8 (overlays, since a card programme is financial regulation by definition, so the regulator half is always a yes and the regulated overlay fires only where a model makes a decision inside the programme, per the rule in [os/STAGE-GATES.md](../../os/STAGE-GATES.md)), DESIGN-2 (integrations, since the sponsor bank, network, and wallet providers can each unilaterally stop the programme), DELIVER-6 (regulated overlay drift, since sponsor terms and network rules can change between sign-off and launch), and OPERATE-4 (cost to run, where dispute volume and programme exposure are the operational load unique to issuing).
...
**Filled in this repo:** [domain-card-issuing-risk-register.md](../../examples/domain-card-issuing-risk-register.md) fills the [risk-register](../../templates/execution/risk-register.md) template directly for this domain, for Saltmarsh Prepaid's consumer prepaid programme: the Regulation E error-resolution clock with provisional credit at 10 business days, a network rule bulletin, a processor outage, token provisioning failures, and a sponsor-bank supervisory action, with risks owned jointly by the programme manager and the sponsor bank's compliance officer and accepted only with sponsor sign-off. For the other three bent templates, [harbourgate-integrations.md](../../examples/harbourgate-integrations.md) shows the counterparty-SLA-and-failure-behavior row shape a sponsor, network, or wallet row needs, and [harbourgate-release-readiness.md](../../examples/harbourgate-release-readiness.md) shows a phased-cohort GO WITH CONDITIONS decision, the shape a programme launch or limit change needs with the sponsor bank's sign-off as its own line. No filled eval-spec example in this repository was written for an underwriting or fraud model; [domain-hr-tech-eval-spec.md](../../examples/domain-hr-tech-eval-spec.md) is the nearest filled eval-spec for its threshold-and-slice discipline, and [expense-copilot-prd.md](../../examples/expense-copilot-prd.md) is near only because it writes acceptance criteria around a model's behavior in an unrelated PRD.

<!-- generated: example-availability -->
**Example availability (generated).** Of the 4 templates this card bends, 4 have a filled example somewhere in this repository: [integrations](../../templates/architecture/integrations.md) (2), [eval-spec](../../templates/ai/eval-spec.md) (2), [release-readiness](../../templates/delivery/release-readiness.md) (3), [risk-register](../../templates/execution/risk-register.md) (6). Written for this domain: [domain-card-issuing-risk-register.md](../../examples/domain-card-issuing-risk-register.md). Filled examples of [integrations](../../templates/architecture/integrations.md), [eval-spec](../../templates/ai/eval-spec.md), [release-readiness](../../templates/delivery/release-readiness.md) exist, but none was written for this domain. This paragraph is generated by `python3 tools/example_availability.py`.
<!-- end generated: example-availability -->

**Worked example (ILLUSTRATIVE):** a filled risk-register row for a fictional programme, Saltmarsh Prepaid, operating under a sponsor bank's BIN, showing how the sector turns a generic delivery risk into a dispute-timeline risk.

| # | Risk (event, not a vague noun) | Category | L | I | Score | Response | Mitigation and its trigger | Owner |
|---|---|---|---|---|---|---|---|---|
| R9 | A cardholder's Regulation E dispute misses the 10-business-day provisional-credit deadline because the case queue backs up during a fraud spike | compliance | 2 | 3 | 6 | mitigate | Auto-escalate any open case to a second reviewer at business day 8; the sponsor bank's own dispute-SLA dashboard alerts at day 9; trigger is any case crossing day 8 with no provisional credit posted | Head of Disputes |

The generic register would score this the same as a shipping delay. The sector fact that changes the row is the fixed regulatory clock: the mitigation has to fire before day 10, not after, because there is no grace period to negotiate with an examiner the way there is with a customer.
