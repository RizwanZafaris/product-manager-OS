...
**Adjacent industries:** carbon-accounting and climate-disclosure software reads this card, because its output is a regulated disclosure an assurance provider can refuse to sign, just like a ledger. EU CSRD reporting under ESRS (scope cut back by the 2025 Omnibus package; verify current thresholds), ISSB-based regimes and California's climate-disclosure laws set what must be reported, and Scope 3 figures under the GHG Protocol lean on supplier estimates and spend-based emission factors, so data provenance and methodology versioning are requirements; the buyer is the CFO or controller. Consumer and SMB tax and bookkeeping software reads this card too, with two additions. In the US, IRC section 7216 restricts using or disclosing tax-return information without specific consent, which bounds marketing, cross-sell and model-training features, and IRS e-file provider rules gate transmission. And the product runs to a hard annual deadline with federal and state content rewritten each year, so the season is the release train and an accuracy guarantee is a liability line. Non-profits needing restricted-fund accounting read this card as well. As of 2026-09-11; verify and confirm with counsel.
...
**Conductor overlay:** this domain sharpens DISCOVER-2 (the evidence is the customer's actual data, not their process documentation), DEFINE-5 (how requirements fail: the non-functional requirements are audit requirements, so their thresholds come from a framework rather than from a preference), DESIGN-4 (the premortem's first answer in this domain is the cutover, so migration is designed and owned here rather than left to delivery), and DELIVER-2 (release readiness is dominated by the reconciliation, the rehearsed rollback, and the calendar).
...
**Filled in this repo:** [domain-erp-nfr.md](../../examples/domain-erp-nfr.md) fills the [nfr](../../templates/definition/nfr.md) template directly for this domain: a period-close lock and reconciliation-variance requirement, a segregation-of-duties row, an immutable-ledger audit-logging row, and a split e-invoicing pair (receipt already in force, issuance phased) are the shape a ledger-integrity NFR takes. The other three bent templates still come from a non-ERP product, so read them for mechanics rather than for ledger, audit-trail or e-invoicing content. [harbourgate-migration-cutover-plan.md](../../examples/harbourgate-migration-cutover-plan.md) fills the migration-cutover-plan template directly; its rehearsal-failure and rollback sections are the closest existing account of what this card calls the central artifact of the domain, though the cutover it documents is a payment-provider switch, not a ledger migration, so it carries no reconciliation-to-the-cent step. [harbourgate-release-readiness.md](../../examples/harbourgate-release-readiness.md) fills the release-readiness template directly, showing a Gate 5 sign-off carrying a named condition, the shape a controller's reconciliation sign-off would take, though this example has no reconciliation line to show. [harbourgate-risk-register.md](../../examples/harbourgate-risk-register.md) fills the risk-register template directly; its R5 row (a legacy-table-shape risk with a dated mitigation and a named owner) is the nearest pattern to a go-live-date or data-quality risk row.

<!-- generated: example-availability -->
**Example availability (generated).** Of the 4 templates this card bends, 4 have a filled example somewhere in this repository: [nfr](../../templates/definition/nfr.md) (9), [migration-cutover-plan](../../templates/delivery/migration-cutover-plan.md) (1), [release-readiness](../../templates/delivery/release-readiness.md) (3), [risk-register](../../templates/execution/risk-register.md) (6). Written for this domain: [domain-erp-nfr.md](../../examples/domain-erp-nfr.md). Filled examples of [migration-cutover-plan](../../templates/delivery/migration-cutover-plan.md), [release-readiness](../../templates/delivery/release-readiness.md), [risk-register](../../templates/execution/risk-register.md) exist, but none was written for this domain. This paragraph is generated by `python3 tools/example_availability.py`.
<!-- end generated: example-availability -->

**Worked example (ILLUSTRATIVE):** two NFR rows for an auditor-facing requirement, in the nfr template's own table shape:

| Requirement | Target (number) or owner for the number | Measured how and where | Verified by |
|---|---|---|---|
| No user may initiate, approve and post the same journal entry (segregation of duties) | Zero same-user initiate-approve-post sequences per period (ILLUSTRATIVE target), enforced by a system control, not a policy memo | Segregation-of-duties conflict report, run nightly against the posting log | Corporate Controller, at period close; SOX control-matrix owner |
| General ledger change history is immutable | Every edit or deletion produces a new time-stamped row; no UPDATE or DELETE against the ledger table (ILLUSTRATIVE target, modeled on the audit-trail-in-lieu-of-write-once-storage standard this card's reading list names, SEC Rule 17a-4 as amended in 2022) | Schema-level constraint, sampled quarterly by internal audit | External auditor, at the year-end walkthrough |

The second row is the one a generic NFR sheet would write as "the database should be auditable"; here it names the standard, the mechanism and the signer, because the card's own rule is that the Controller and the auditor are acceptance criteria, not stakeholders to inform.
