...
3. Mark every checklist line pass, fail, or unknown, each with its evidence beside it: a workspace path and section, an evidence-ledger row, or an accepted-answer ID. An unknown stops the Conductor exactly as a fail does, though the `pmos` runtime does not enforce that: it records a gate approval without reading these lines, so a gate can read approved with lines unmet or unjudged.
...
- **Method 3 (agent runtime with file access).** The full protocol as written. The Conductor edits workspace files directly and runs `python3 tools/ci_gate.py` (the full release suite; `lint.py --os` is one gate inside it) before reporting any repo change consistent.
- **Method 4 (local PMOS runtime).** The Conductor runs as code in `pmos/`, which enforces the mechanical parts of this protocol instead of trusting a model to remember them: one offered question at a time, rule 3's two-challenge cap, the park itself, and a gate that cannot be proven while any answer in the bank is parked. The runtime always parks on the third invalid submission; it has no call that accepts an answer as offered, so that half of rule 3's outcome is a by-hand step. The runtime reopens a parked question (Conductor.reopen and `pmos reopen`), keeps every rejected submission and the reopen in an append-only history, and refuses a later answer that repeats rejected evidence. One route above is still not implemented there: the runtime records no waiver, so the escape hatch is run by hand or not at all, and inside the runtime a bank holding a parked answer does not reach its gate until the question is reopened and answered.
- **Method 5 (routed tiers).** Stage work splits across the tiers in [../routing/README.md](../routing/README.md) by blast radius: transcription and smart-skip lookups to the cheap tier, template drafting to the coding tier, cross-examination, gate evaluation, premortem, red team, and persist-pivot-sunset framing to the reasoning tier. Queue when the judgment tier is capped; never downgrade a cross-examination to the cheap tier, because an interrogation that cannot spot a weak answer is worse than a delayed one.
