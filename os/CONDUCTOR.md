...
- **Method 4 (local PMOS runtime).** The Conductor runs as code in `pmos/`, which enforces the mechanical parts of this protocol instead of trusting a model to remember them: one offered question at a time, rule 3's two-challenge cap, the park itself, and a gate that cannot be proven while any answer in the bank is parked. The runtime always parks on the third invalid submission; it has no call that accepts an answer as offered, so that half of rule 3's outcome is a by-hand step. Two of the routes above are not implemented there. The runtime has no call that re-opens a parked question and none that records a waiver, so gate step 5's re-opened questions and the escape hatch are run by hand or not at all, and inside the runtime a bank holding a parked answer does not reach its gate.
