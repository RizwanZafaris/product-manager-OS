...
The tool is a convenience, not a dependency. A workspace made with `mkdir` and `cp` is the same workspace, and the only thing you take on is fixing the links yourself. Everything below holds either way.
...
**What checks it.** `python3 lint.py --workspace products/ledgerline` reports under `ARTIFACT` a copy in a stage folder with no block (naming `--stamp` as the fix), a missing key, a `phase`, `gate` or `status` outside its vocabulary, a `depends_on` that is not a list of IDs, an ID two files share, and a dependency the workspace does not contain yet. That last one is on purpose: a vision copied before its problem framing says so, rather than implying evidence that does not exist. It also reports under `UAT` a filled copy of the UAT plan that claims UAT is finished while one of its charters failed with no defect record and no approved exception, or whose charters were run against a build that is not the candidate the sign-off accepts. It reads a copy as claiming completion once an exit-criteria box is ticked, a sign-off verdict is filled in, or the closing exit gate is signed, and it finds that section by its number or by its wording, so renaming the heading does not turn the check off and a plan still being written is still left alone. It finds the copy itself by its filename, by its `template:` field, or by the sign-off binding it carries, so renaming the file does not turn the check off either. It reads the charter table by its column names, one name per column: a table offering two cells for one column, such as `Expected result` beside `Actual result`, is reported as an ambiguity rather than read from whichever cell comes first, and a table whose cells hold Pass, Fail or Blocked is read wherever in the document it sits. Workspace mode is opt-in, so an unfinished draft never fails the repository's own build.
