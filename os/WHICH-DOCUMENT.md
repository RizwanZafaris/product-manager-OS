...
- **The product contains an AI or machine-learning feature and a financial or data regulator applies to it.** The regulated module governs and its own template is used as shipped. Both halves are required: the module covers two AI-specific instruments and nothing else, so a regulated product with no model in it brings its own licence conditions, scheme rules, and regulatory calendar to Gate 2 instead. See [modules/regulated/README.md](../modules/regulated/README.md#when-this-overlay-activates), which states the same condition, as a truth table; the narrowed rule in [STAGE-GATES.md](STAGE-GATES.md) governs. A regulated feature never runs at ticket weight, whatever its size: this is the one place in this file where size does not decide. A three-line edit to a disclosure string is a small change and a regulated artifact at once, and the artifact is what a supervisor asks for two years later, when nobody can remember whether the wording was reviewed.
...
- **Design critique outcome.** Not a document at all: a critique's outcome is a logged decision in the [decision log](../templates/execution/decision-log.md), never its own write-up. The signed artifact is the [design review record](../templates/architecture/design-review-record.md), a different thing run at a different moment.
- **Component spec, for every component.** Write [component-spec.md](../templates/architecture/component-spec.md) only for a component shared by two or more surfaces; a component built once for one screen belongs to that screen's own artifacts and never earns its own file.
