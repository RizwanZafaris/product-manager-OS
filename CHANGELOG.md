...
- **A commercial transition contract in `templates/planning/pricing-packaging.md`.** The blank decided a price and stopped there: a value metric, tiers, a benchmark and discount rules, and no trial, overage, upgrade, downgrade, proration, refund or grandfathering terms, no per-tier evidence or cost floor, and nowhere to record who moves when a price changes. Section 3's tier table gains a `Price evidence` column and a new section 3a, the economic floor per tier, reading the cost to serve and the contribution margin from [frameworks/metrics/unit-economics.md](frameworks/metrics/unit-economics.md) lines B, C and D. Three sections are added: 7, the standing commercial terms, one row per term with the row that says "not offered" required rather than optional; 8, the change record, one row per moving cohort answering who moves, what they pay now and after, the notice, the effective date, the reversal and the evidence, above a migration-applicability line that either names the cutover plan the change runs under or says why none is needed; and 9, a checklist routing billing rules to [the business rules register](templates/definition/business-rules.md), the notice to [the launch comms plan](templates/delivery/launch-comms-plan.md) and [customer comms](templates/delivery/customer-comms.md), the move itself to [the migration cutover plan](templates/delivery/migration-cutover-plan.md), contracted prices to [the change request](templates/execution/change-request.md), the first changed invoice to [the support runbook](templates/delivery/support-runbook.md), and the decision to [the decision log](templates/execution/decision-log.md). The new sections are numbered 7 to 9 and sit after the review section rather than beside the price: documents across this tree, in `frameworks/`, `examples/` and `knowledge/`, cite sections 1 to 6 of this file by number, and renumbering them would have repointed every one of those citations silently. Four exit-gate lines are added. `skills/pricing-packaging/SKILL.md` step 5 and its exit gate now name the sections its migration rules land in.
- **A check inside the docs contract, `pricing-contract`.** `python3 tools/docs_contract.py` now reads the pricing blank's section 3 for the column both pricing worksheets say they feed, reads section 7 for every standing term, reads section 9's routes and checks each routed file exists, reads section 3a and `frameworks/metrics/unit-economics.md` for the per-tier economic floor each promises the other, and reads section 8 in both the blank and `examples/ledgerline-pricing-packaging.md` for the seven things a change record has to ask. Everything is read with the HTML comments taken out, so guidance a reader never sees does not count as a field. `tests/test_tools_gates.py::PricingContractGateTests` is fifteen tests: thirteen seed one defect each and assert it is reported, and all thirteen fail when `check_pricing_contract` is stubbed to return nothing. The other two are the positive control on the tree as it stands and the case of a tree with no pricing blank, which makes no claim; neither fails on that revert, because neither is meant to. Stated limits, also written beside the check: it counts headings and column names and does not read what is written under them, so a change record whose cells are all unanswered placeholders passes here and is caught by the people who sign Gate 5; and only the one declared filled example is read, because the declaration line is the only thing in the tree that says which template an example fills, and it is read for its change record alone, that being the thing the finding asked an example to prove.
...
- **`frameworks/pricing/van-westendorp.md` promised a column that did not exist.** Its Feeds line sent the acceptable range to "section 3 (tiers), the range as the evidence column", and section 3 had five columns, none of them evidence. It now names the `Price evidence` column that exists, and says the range is quoted as an interval with its n and its fielding date. [frameworks/pricing/gabor-granger.md](frameworks/pricing/gabor-granger.md) and [frameworks/metrics/unit-economics.md](frameworks/metrics/unit-economics.md) name the same column and the new section 3a. The `pricing-contract` check pins the column's name in one place, so renaming it in the template alone, or in a worksheet alone, is a failure rather than a fresh silent promise.
- **`examples/ledgerline-pricing-packaging.md` carries the worked price change.** It fills the new sections dated 2027-01-12, after EV-S01 was read, and it is dated as an addition rather than backdated into a document signed 2026-11-03. Section 8 covers four cohorts: the 73 EXP-1 payers held at $6 a seat until each account's own twelve-month hold ends between 2027-11-23 and 2027-12-18, then the proposed $2.40 a drafted report, about a 92% to 96% fall against the $204 a median account pays now; the six design partners at 25% off until their terms end from 2027-11-10; the roughly 2,818 to 2,821 accounts not paying, for whom the offer page carries the review wording from 2026-12-21; and the two Enterprise quotes withdrawn that day. Each row carries its notice, its effective date, its reversal and its evidence, and the two billing behaviours the dates depend on, the seat meter's month-close reading and the twelve-month contracted-price rule, are written out in section 7 instead of being assumed. Section 7 also records what was never decided: the timing of a mid-term downgrade and the refund rule, neither of which has ever been exercised. Section 9 records that three of its seven rows are not done, two of them not yet needed. The exit gate grows from six lines to ten, and five are open where two were open at signing; four of the five are gaps the document had all along and could not previously record.
