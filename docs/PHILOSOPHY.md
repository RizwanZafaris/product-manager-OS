...
**What the tree actually does about it.** It splits the layers so degradation is partial rather than total. Losing the model costs you the interview, the drafting, and the routing; it does not cost you the format, the gates, or the record, because those are files. The dependency direction enforces it: skills cite templates, templates never cite skills as a requirement.

**Where the belief is weaker than it sounds, tested 2026-09-03.** Two claims used to travel together here and only one of them is a guarantee. Deleting `harness/` leaves the document gates green and five of the twenty-one release gates red, measured in the [README's quality-gate section](../README.md#quality-gate). Deleting the content layers, `skills/`, `agents/`, `system/`, and `routing/`, leaves documents that still take a product from Gate 1 to Gate 6, and leaves the structural lint gate failing in the hundreds, because each template's `Skill:` header is a markdown link and unresolved links are exactly what that gate checks. Stripping the upward references out of every template that carries one would buy the stronger sentence at the cost of the cross-referencing that makes the tree teachable, which is the worse trade, so the limit is written down instead: usable with no model, not lint-clean with a layer missing.
...
**What the tree actually does about it.** It scopes the date check to the material where the calendar genuinely changes the answer, which is the regulated overlay, and it keeps the failure cheap to clear: re-verify, restate the date, move on. Everywhere else the honesty mechanism is disclosure rather than enforcement, which is why the version policy promises that field names and paths do not move under you inside a major version, and why the gate is documented to mean one thing only: green says the tree is consistent, never that anything in it is true.
