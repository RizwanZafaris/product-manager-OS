...
| ChatPRD | Hosted commercial AI PM product | Drafting and coaching a PM through product documents in a browser, with a company behind it | Being usable in five minutes by someone who does not open a terminal. Hosted collaboration, a maintained product roadmap, mobile access, and a coaching tone that is genuinely good for a first-time PM. There is a support address to email. | Inspectability and ownership. Every prompt here is a file you can read, diff, fork, and pin to a version; nothing here calls out on its own, and the only code that makes a network call is optional and runs when you invoke it: the legacy `harness/runner.py` against a gateway you chose, the runtime's OpenRouter adapter, and `tools/ext_ai_probe.py`, the evidence probe that drives that adapter or a loopback OmniRoute gateway; and the artifacts are yours in plain markdown with no subscription attached |
