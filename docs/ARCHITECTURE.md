...
├── tools/
│   ├── ci_gate.py  The canonical release suite, twenty-two named gates in one pass; CI invokes it first, and --manifest prints the gate list the readiness evaluator checks against
...
│   ├── template_rubric.py  Scores every template against the flagship bar derived from prd.md
