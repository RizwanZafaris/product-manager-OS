...
The shortest honest description is a library plus a discipline. The library is the templates, cards, and worksheets. The discipline is that opening a stage before the previous gate passes on evidence costs you a written waiver naming who insisted and what is still unmet, and that the size of the document is chosen before the document is opened. Discipline is the right word and control is not: nothing here can hold a stage shut against someone determined to open it.

**2. Do I need AI to use it?**

No, and this is load bearing rather than a hedge. Method one of the five in [os/CONDUCTOR.md](../os/CONDUCTOR.md#per-method-notes) uses no model: clone, copy a template, fill it in any editor, work the gate checklist by hand. No document under `knowledge/`, `frameworks/`, `templates/`, or `os/` needs an AI layer to be readable or fillable, and the direction of dependency runs downward: a skill requires its template, a template does not require its skill.

**What is actually proved, and what is not.** `harness/` is deletable, and CI proves it on every push: the `deletable-harness` job in [.github/workflows/lint.yml](../.github/workflows/lint.yml) deletes `harness/` and runs three checks on what is left, `lint.py --os`, `tools/check_manifest.py` and `tests/test_lint.py`, all green. That is the whole of the proof. The job does not run the other gates in `tools/ci_gate.py`, so this file makes no claim about how many of the 25 gates survive the deletion; it used to name a number for that, measured against a tree with fewer gates, and a number nothing re-measures is worse than no number. What earns the four that pass is a rule rather than luck: a file outside `harness/` names a harness path in backticks, never as a link, because a link is a dependency and the link gate treats it as one. The two links in `AGENTS.md` that broke the earlier run recorded in `harness/README.md` are now backticked paths, so that section records the failing run and this one records the fix. The document layers are a weaker claim than this file used to make. Delete `skills/`, `agents/`, `system/`, and `routing/` and the remaining documents still take a product from Gate 1 to Gate 6, because every template's guidance is prose and its `Skill:` header is a pointer rather than a requirement. The same deletion breaks the structural lint gate in the hundreds, because those pointers are markdown links and links are what the gate checks. So the load-bearing sentence is this one: you can run the whole loop without a model, and you cannot delete a content layer and keep a green gate. Delete `templates/` instead and the AI layers have nothing left to produce, which tells you which half is the product.
...
Often, yes, and the tell sits in your own gate folder: if every gate passed on attempt one, it is already theater. The design defenses are that the gate names a person with standing to stop the stage, which is a different person from the one running it, that each gate carries a written skip-risk warning so waving it through means waving through something specific, that going past a failed line costs a waiver naming who insisted and who objected, and that failed attempts are filed rather than deleted.

**What those defenses are worth, exactly.** All four are prose conventions a person follows or does not. There is no hash tying a signature to the document version it approved, no check on a typed name, and nothing preventing the author of an artifact from signing it off, so a determined person defeats any of them in a minute. The defenses buy one thing, and it is the thing theater cannot survive: a document that nobody reviewed looks unreviewed on the page, a line nobody could evidence stays marked unknown, and a gate somebody skipped carries a name and a date instead of silence. Read as a control, this system is weak. Read as a record that makes the skipping legible to the next person, it holds.
...
The document system through 0.7.1 was written with Claude Code by a working payments CPO. The 0.8.0 local runtime hardening was implemented and red-teamed with OpenAI Codex agents. Hiding either would be the exact defect this repository claims to guard against. What that means in practice matters more than the label: the structure, beliefs, skip conditions, and failure modes come from the maintainer's product and payments work, while models contributed drafting, implementation, consistency checks, and adversarial review.

**How to check the claim rather than take it:** read the commit history. AI-assisted non-merge commits identify their tool in trailers, the messages show what was rewritten and when, and the [changelog](../CHANGELOG.md) records which release studied which prior art before writing. A repository hiding its authorship would have no reason to keep any of that legible.
...
**The things that decay anyway.** Regulated citations go stale, and the gate is built to fail on the date rather than hide it. [COMPARISON.md](COMPARISON.md) ages, which is why it carries a comparison date and a re-check procedure. An optional provider integration can also change or disappear; the local runtime's model catalog is intentionally discovered at call time rather than treated as a permanent list. Forking is the intended succession plan, not a fallback.
...
- **Deletions the document gate supports.** `harness/` only. Remove it and `python3 lint.py --os`, `python3 tools/check_manifest.py`, and the graph check still pass, which is the run recorded in `harness/README.md`. The optional `pmos/` runtime is separate: deleting it leaves the document path intact but removes the local CLI, transactional state, and local runtime checks.
- **Deletions that keep the documents and cost you the gate.** `learn/`, `examples/`, `routing/`, `agents/`, `skills/`, `system/`, `modules/regulated/` when your product has no AI or machine-learning feature under a supervisor (its two cited instruments are both about AI, so a non-AI regulated product gets nothing from it and needs its own jurisdiction pack, which this repository does not ship), and any domain or knowledge card outside your market. Nothing depends upward on these, so every remaining template still fills and every gate still runs by hand. They are also link targets, so the lint gate then reports the links that no longer resolve, in the hundreds for the four AI layers. Fine if you never run the gate; a fork that wants a green gate has to remove the pointers too.
...
Any capable chat model runs the boot prompt, free tiers included, which is why [system/BOOT-PROMPT.md](../system/BOOT-PROMPT.md) assumes no file access and asks you to paste files by exact path. In an agent CLI, the skills and templates load themselves. The cost lever is the tier doctrine in [routing/README.md](../routing/README.md): extraction and reformatting on a cheap tier, drafting on a coding tier, judgment on a frontier tier. The optional local runtime has no model requirement; it can use its OpenRouter adapter only when you explicitly configure an environment-only key.

**The rule behind it:** pay for reasoning where a wrong answer propagates through a gate, and pay nothing extra where the work is rearrangement, because a frontier model reformatting a table is waste and a cheap model critiquing a gate is worse than not asking. OpenRouter models are discovered dynamically; a currently free model may be absent, rate-limited, or change capability. A free model is never judgment-grade merely because it answered. High-risk work requires an operator-certified model and the matching privacy permission; OmniRoute remains an optional alternative, not a dependency.

**16. Who owns my data and my output?**

You do, on both counts. The document path and local runtime have no telemetry, account, or hosted service, and neither calls out on its own. `pmos` stores runtime state locally under the workspace's `.pmos/`; its offline provenance records hashes and metadata, not artifact contents, environment variables, or credentials. A provider call happens only when an operator invokes one: the legacy harness can post to an OmniRoute endpoint, and the runtime can call OpenRouter using `OPENROUTER_API_KEY` read only from the environment at request time. Whatever you paste into a model goes wherever that vendor's terms say it goes, which is between you and them. If product data cannot leave your environment, use the document or local-runtime path without a provider. Filled artifacts live in `products/<name>/`, which is gitignored and never shipped here, so your work cannot collide with an update or be committed into this tree by accident. See [../SECURITY.md](../SECURITY.md).
