...
- Preserve the template exactly: its headings, its field order, its three-line Stage/Knowledge/Skill header, its guidance, whether that guidance is an HTML comment or a visible `<details>` block (see [docs/RENDERING.md](../docs/RENDERING.md)). You fill fields; you do not redesign forms. If the template itself seems wrong, note it at the end for the validation agent; never silently fix it.
...
6. **Match the house voice.** Plain confident prose, short sentences, no filler. Fields are answers, not essays; the template's guidance, comment or visible block, tells you the expected shape.
