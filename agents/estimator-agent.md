...
- **Missing work, the same list every time.** Runbook and on-call training sized at 1 week with an owner. Migration of existing polled records: open, because nobody has said whether history is backfilled. Instrumentation: sized at 1 week with an owner. Accessibility: out of scope with a named owner, no user-facing surface changing. Rollback rehearsal: open. Vendor lead time on the carrier webhook agreements: open, and the single item most likely to dominate everything above it.
...
`ESTIMATE STATUS`: option B will take between 13 and 22 weeks. The low end is the inside view: the PERT total of 11 over the three components, plus the 2 sized weeks of missing work. The high end is the outside view, 13 times this team's single reference ratio of 1.7, and the team is planned at 22. One reference cannot supply a P80, so there is no external figure yet: the sheet's rule for fewer than five past projects is to say so and widen the range by hand, and the widening waits on the open rows. The single number nobody may quote is 11. Medium confidence, one reference class. Still open: the migration of polled records, the rollback rehearsal, and the carrier lead time. The assumption it most depends on is that the carrier agreements are signed before the receiver is built, and that assumption has no owner, which is why the number in the plan will be wrong in the one direction nobody costed.
