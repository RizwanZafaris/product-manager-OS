...
| DEFINE | drafting-agent | research-agent; [acceptance-agent](acceptance-agent.md) for criteria that can fail; [estimator-agent](estimator-agent.md) for the first sizing; validation-agent; the design-review skill, as one evaluator | Gate 2: product owner, engineering lead, business sponsor; regulatory owner where regulated | One-pager or BRD, PRD, FRD; NFR; acceptance criteria; assumptions register |
