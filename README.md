...
`modules/regulated/` is a verbatim import of the regulated AI PRD system: a section-0 regulatory overlay, eval-set acceptance criteria, guardrails with owners, and its own review gate. Its canonical source is [github.com/RizwanZafaris/regulated-ai-prd](https://github.com/RizwanZafaris/regulated-ai-prd). The citation-bearing files inside it are byte-exact copies, pinned by hash in the quality gate, and are never edited in this repository. Fixes happen in the source repo and are re-copied here. The overlay activates at Gate 2 and Gate 5 whenever the product operates under a financial or data regulator; see [modules/regulated/README.md](modules/regulated/README.md).
