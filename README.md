...
Within a major version, template field names and file paths do not change under you. A copy you filled in last quarter keeps matching the template it came from, and a link you wrote into your own documents keeps resolving. Renaming a field, or moving or deleting a linked file, is a breaking change; those happen only on a major version, and each one is named in [CHANGELOG.md](CHANGELOG.md) with the migration beside it.

**Three things carry versions here, and they are not the same thing.** The *package version* is the `version` field of `pyproject.toml` and the tag cut from it, and it covers the tree as shipped. The *document schema* is the templates together with the checks that read a filled document, chiefly `REQUIRED_SECTIONS` in `lint.py` and the nine required sections `pmos handoff` reads; it carries no version number of its own and moves with the package. The *runtime bank contract* is `pmos/question_banks.json`, generated by `tools/question_banks.py`, in which every question bank carries its own version, plus a definition hash computed from the questions it asks. Changing what a gate demands means a different thing on each of these, which is why one sentence cannot promise for all three, and why the two paragraphs that used to stand here contradicted each other.

**The document schema is checked as it stands today.** A filled document is read by the current checker, not by the checker it was filled against, because nothing in a filled Markdown file records which template shape produced it. So a section a minor release adds to a template and no check requires leaves an older filled document passing, and a section that same release also adds to `REQUIRED_SECTIONS` makes that document fail with a missing-section error. On this surface an added mandatory section is a breaking change, and it belongs to a major version. Two minor releases, 0.6.0 and 0.5.1, made one anyway; their entries in [CHANGELOG.md](CHANGELOG.md) carry dated notes saying so, and the rule is written here so the next one is named as breaking rather than left inside the word minor. What a minor release does promise here is narrower: your fields are not renamed, your paths still resolve, the document still renders, and it still records what was decided. Keep an older filled document as that record, and diff it against the current template before it has to pass a gate again.

**The runtime bank contract is pinned per product.** A product records each bank's version and definition hash in its Conductor state, and the Conductor refuses to open a product whose recorded banks differ from the banks it is handed. A shipped contract change therefore does not reach an existing product at all: it stays on the questions it started with, and its approvals keep holding, until someone runs `pmos repin`. An approval recorded since contract pinning also stores the hash of the questions its bank asked, which is what makes a later change visible instead of silent. On this surface an added mandatory question is not a breaking change for a product already in flight, because no product adopts it without being told to.

**Migration and reapproval.** `pmos repin --dry-run` reports what adopting the shipped contract would change and writes nothing; `pmos repin` adopts it and keeps every stored answer. A question added to a bank the product has not yet approved is adopted and asked when the interview reaches it, and no approved gate goes stale. A question added to a bank the product has already approved is refused: repin names the bank and the question ids, writes nothing, dry run included, and the product keeps the banks it started with. Finish that product on the contract it has, or start the next one on the current contract. A question reworded in an approved bank is adopted and its answer kept, and that bank's gate reports stale until it is proved again with `pmos gate`; that re-proof is the reapproval step, and the approval it replaces is kept as a superseded record rather than deleted. A gate approved before contract pinning carries no question hash and does not go stale on a contract change. [docs/RUNTIME-QUICKSTART.md](docs/RUNTIME-QUICKSTART.md) walks through the commands, and `tests/test_versioning_contract.py` drives an already approved fixture through each of these outcomes.
