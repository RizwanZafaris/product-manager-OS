...
**Hosted CI on the default branch is currently red, for one reason.** The exact-tree review record in [docs/readiness/independent-review.json](docs/readiness/independent-review.json) binds to a tree digest, and the tree has moved since a reviewer last accepted it, so the check reports the acceptance as stale. Every other gate in the release suite passes. That is the check working rather than failing: an acceptance covers the tree that was read, and this one no longer describes this tree. It clears when somebody who did not write the change reads it and runs `python3 tools/review_gate.py --record`, which is the whole of [the review brief](docs/readiness/EXT-TEAM-review-brief.md).
